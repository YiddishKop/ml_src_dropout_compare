...
data) as judged by cifar10_DSD.eval.py.
...
def eval_once(saver, top_k_op, keep_prob, epoch, mask_placeholder, mask):
  ...
  with tf.Session() as sess:
    ...
    if ckpt and ckpt.model_checkpoint_path:
      ...
      #   /my-favorite-path/cifar10_DSD.train/model.ckpt-0,
...
def evaluate(step, mask):
  ...
  with tf.Graph().as_default() as g:
    ...
    images, labels = cifar10_DSD.inputs(eval_data=eval_data)
    keep_prob = tf.placeholder(tf.float32)

    # Build a Graph that computes the logits predictions from the
    # inference model.
    _, mask_placeholder, logits = cifar10_DSD.inference(images, keep_prob)
    ...
    variable_averages = tf.train.ExponentialMovingAverage(
        cifar10_DSD.MOVING_AVERAGE_DECAY)
...
def Sparse_to_ReDense(weights,name,shape,mon_sess):
  """Assign the zero weights to ReDense layer.
  ...
  with tf.variable_scope(name, reuse=True):
    network_weights = tf.get_variable('weights', shape)

  mon_sess.run(tf.assign(network_weights, weights))
...
def train():
  ...
  with tf.Graph().as_default():
    ...
    # Get images and labels for CIFAR-10.
    images, labels = cifar10_DSD.distorted_inputs()
    keep_prob = tf.placeholder(tf.float32)

    # Build a Graph that computes the logits predictions from the
    # inference model.
    network_weights_placeholder, mask_placeholder, logits = cifar10_DSD.inference(images, keep_prob)

    # Calculate loss.
    loss = cifar10_DSD.loss(logits, labels)

    # Build a Graph that trains the model with one batch of examples and
    # updates the model parameters.
    train_op = cifar10_DSD.train(loss, global_step)
